...
public class CreateManualMininetTopologyDpctl {

	//To set IP to hosts in Mininet
	private String IP="10.0.0.0";

	public CreateManualMininetTopologyDpctl(){
		...
		try
		{
			...
			while(i<numEdges){
				if(edges_arquivo[i][campoEdgeType].equals("TOR"))
					topologiaMininet.append("\n\t\tself.addLink(h"+edges_arquivo[i][campoEdgeFrom]+", s"+edges_arquivo[i][campoEdgeTo]+")");
				else
					topologiaMininet.append("\n\t\tself.addLink(s"+edges_arquivo[i][campoEdgeFrom]+", s"+edges_arquivo[i][campoEdgeTo]+")");
				i++;
				j++;
			}//fim while
	...
	public void gerarRegrasOpenFlow(){
		...
		System.out.println("\n---Manual: Creation of OpenFlow rules---\n");
		...
		try
		{
			...
			//	   lisFlows[1][1]=100
			//     ...
			String[][] listFlows = new String[i][4]; //-1 because last increment is not considered
			...
			while(token.hasMoreElements()){
				...
				System.out.println("SourceNode: ["+listFlows[i][0]+"] DestinationNode: ["+listFlows[i][1]+"] AggregatedTraffic: ["+listFlows[i][2]+"] ManualPath: ["+listFlows[i][3]+"]\n");
			...
			//---------------------Inicio da Geracao das regrasAdd---------------------						
			i=0;
			j=0;

			//Armazena as regrasAdd OpenFlow
			StringBuffer regrasAdd = new StringBuffer();
			...
			i=0;
			while(i<listFlows.length){				
				PATH_after[i].append(listFlows[i][3]);
				System.out.println("----");
				i++;
			}//end while
			...
			while(i<PATH_after.length){
				...
				if (t_split.countTokens()==2){
					...
					//System.out.println(regrasAdd.toString());
					//System.exit(0);
			...
			//---Fim da Geracao das regrasAdd---

			//Grava em arquivo 
			try {
				BufferedWriter out = new BufferedWriter(new FileWriter(conf.getArquivoRegrasOpenFlow(),false));
				out.write("\n#--- OpenFlow1.3 Rules: Automatically Generated by MILPFlow Version " + conf.getVersion()+" ---");
				out.write("\n#");
				out.write("\n#--- How to run Mininet: ---");
				out.write("\n#");
				out.write("\n# mn --mac --custom " +
						"modeloMininet_"+conf.getNumServers()+"serv_"+conf.getNumVMs()+"vm.py "+
						"--topo " +
						"modeloMininet_"+conf.getNumServers()+"serv_"+conf.getNumVMs()+"vm "+
						"--switch user");
				out.write("\n#");
				out.write("\n#--- How to run this rules: ---");
				out.write("\n#");
				out.write("\n# chmod +x "+
						"modeloMininet_"+conf.getNumServers()+"serv_"+conf.getNumVMs()+"vm_regrasOpenFlow.sh");
				out.write("\n#./modeloMininet_"+conf.getNumServers()+"serv_"+conf.getNumVMs()+"vm_regrasOpenFlow.sh");

				//Write rules
				out.write(regrasAdd.toString());
				out.close();
			} catch(Exception e){
				System.out.println("13Excecao ao gravar no arquivo." + e.getMessage());
			}//fim catch
